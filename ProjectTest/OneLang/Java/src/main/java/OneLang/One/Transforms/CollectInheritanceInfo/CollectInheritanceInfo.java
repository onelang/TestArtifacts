...
public class CollectInheritanceInfo implements ITransformer {
    ...
    public void visitClass(Class cls)
    {
        ...
        for (var field : cls.getFields())
            field.interfaceDeclarations = Arrays.stream(Arrays.stream(intfs).map(x -> Arrays.stream(x.getFields()).filter(f -> f.getName() == field.getName()).findFirst().orElse(null)).toArray(Field[]::new)).filter(x -> x != null).toArray(Field[]::new);
        
        for (var method : cls.getMethods()) {
            method.interfaceDeclarations = Arrays.stream(Arrays.stream(intfs).map(x -> Arrays.stream(x.getMethods()).filter(m -> m.name == method.name).findFirst().orElse(null)).toArray(Method[]::new)).filter(x -> x != null).toArray(Method[]::new);
            method.overrides = Arrays.stream(Arrays.stream(clses).map(x -> Arrays.stream(x.getMethods()).filter(m -> m.name == method.name).findFirst().orElse(null)).toArray(Method[]::new)).filter(x -> x != null).findFirst().orElse(null);
