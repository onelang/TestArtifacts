...
public class InferForeachVarType extends InferTypesPlugin {
    ...
    public Boolean handleStatement(Statement stmt) {
        if (stmt instanceof ForeachStatement) {
            var runPluginsOnResult = this.main.runPluginsOn(((ForeachStatement)stmt).items);
            ((ForeachStatement)stmt).items = runPluginsOnResult != null ? runPluginsOnResult : ((ForeachStatement)stmt).items;
