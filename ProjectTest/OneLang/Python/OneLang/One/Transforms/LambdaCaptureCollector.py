...
class LambdaCaptureCollector(astTrans.AstTransformer):
    ...
    def visit_variable_reference(self, var_ref):
        if isinstance(var_ref, refs.StaticFieldReference) or isinstance(var_ref, refs.InstanceFieldReference) or isinstance(var_ref, refs.StaticPropertyReference) or isinstance(var_ref, refs.InstancePropertyReference):
            return None
        if self.scope_vars == None:
            return None
